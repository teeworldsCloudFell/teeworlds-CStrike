...
void CGameControllerOpenFNG::DoInteractions()
{
	...
	for(int i = 0; i < MAX_CLIENTS; i++)
	{
		...
		if (Col == TILE_SHRINE_ALL || Col == TILE_SHRINE_RED || Col == TILE_SHRINE_BLUE)
		{
			...
			pChr->Die(pChr->GetPlayer()->GetCID(), WEAPON_WORLD, true);

			if(pChr->GetPlayer()->m_IsDead != TEAM_SPECTATORS && pChr->GetPlayer()->GetTeam() != TEAM_SPECTATORS)
			{
				pChr->GetPlayer()->SetTeam(TEAM_SPECTATORS,false);
				continue;
			}
			//edited
		}
...
{//assertion: Killer >= 0, victim anyways
	...
	if(!Wrong)
		pVictim->GetPlayer()->m_IsDead = pVictim->GetPlayer()->GetTeam();
	...
	pPlKiller->m_Money += Wrong? 0:(ShrineTeam == -1 ? CFG(MoneyWhiteFlag) : CFG(MoneyTeamFlag));
...
void CGameControllerOpenFNG::SendFreezeKill(int Killer, int Victim, int Weapon)
{
	...
	if(PLAYER(Victim)->m_IsDead != TEAM_SPECTATORS)
	{
...
void CGameControllerOpenFNG::OnCharacterSpawn(class CCharacter *pChr)
{
	...
	pChr->GiveWeapon(WEAPON_KNIFE, -1);
	//pChr->GiveWeapon(WEAPON_RIFLE, 10);
	//pChr->GiveWeapon(WEAPON_GRENADE, 10);
	//pChr->GiveWeapon(WEAPON_SHOTGUN, 10);
	//pChr->GiveNinja();
	pChr->SetWeapon(WEAPON_KNIFE);

	if( pChr->GetPlayer()->m_IsDead == TEAM_SPECTATORS)
	{
		if(pChr->GetPlayer()->m_Upgrades.m_Glock || pChr->GetPlayer()->m_Upgrades.m_Desert || pChr->GetPlayer()->m_Upgrades.m_USP)
		{
			pChr->GiveWeapon(WEAPON_GUN, 10);
			pChr->SetWeapon(WEAPON_GUN);
		}

		if(pChr->GetPlayer()->m_Upgrades.m_FAMAS)
		{
			pChr->GiveWeapon(WEAPON_SHOTGUN, 10);
			pChr->SetWeapon(WEAPON_SHOTGUN);
		}

		if(pChr->GetPlayer()->m_Upgrades.m_RPG)
		{
			pChr->GiveWeapon(WEAPON_GRENADE, 10);
			pChr->SetWeapon(WEAPON_GRENADE);
		}

		if(pChr->GetPlayer()->m_Upgrades.m_AWP || pChr->GetPlayer()->m_Upgrades.m_AK47 || pChr->GetPlayer()->m_Upgrades.m_MP4a)
		{
			pChr->GiveWeapon(WEAPON_RIFLE, 10);
	...
	{
		pChr->GetPlayer()->m_IsDead = TEAM_SPECTATORS;
		pChr->GetPlayer()->Clear();
	}
