...
void CGameContext::CreateDamageInd2(vec2 Pos, float Angle, int Amount, int CltMask)
{
	float s = 0;
	float e = 2*pi;
	for(int i = 0; i < Amount; i++)
...
void CGameContext::SendTuningParams(int ClientID)
{
	...
	Server()->SendMsg(&Msg, MSGFLAG_VITAL, ClientID);*/ //must delete
	//CheckPureTuning();
	if(ClientID == -1)
	{
		for(int i = 0; i < MAX_CLIENTS; i++)
...
void CGameContext::OnClientEnter(int ClientID)
{
	...
	if(m_pController->m_RoundCount)
	{
		m_apPlayers[ClientID]->m_IsDead = m_apPlayers[ClientID]->GetTeam();
		m_apPlayers[ClientID]->SetTeam(TEAM_SPECTATORS,false);
	}
	else
	{
		m_apPlayers[ClientID]->Respawn();
		m_apPlayers[ClientID]->m_Money = g_Config.m_SvMoneyLooser;
	}
...
void CGameContext::OnMessage(int MsgID, CUnpacker *pUnpacker, int ClientID)
{
	...
	else if(MsgID == NETMSGTYPE_CL_CALLVOTE)
	{
		...
		bool IsSpecialVote = false;
        const char* pOptionDesc;
		...
		if(str_comp_nocase(pMsg->m_Type, "option") == 0)
		{
			...
			while(pOption)
			{
				if(str_comp_nocase(pMsg->m_Value, pOption->m_aDescription) == 0)
				{
					...
					if(!str_comp_nocase(pOption->m_aCommand, "specialvote"))
					{
						IsSpecialVote = true;
						pOptionDesc = pOption->m_aDescription;
					}
					break;
				}
		...
		if(!IsSpecialVote)
		{
			if(pPlayer->GetTeam() == TEAM_SPECTATORS && !(pPlayer->m_IsDead != TEAM_SPECTATORS))
			{
				SendChatTarget(ClientID, "Spectators aren't allowed to start a vote.");
				return;
			}
			...
			HandleOptions(ClientID, pOptionDesc, pReason);
	...
	else if (MsgID == NETMSGTYPE_CL_SETTEAM && !m_World.m_Paused)
	{
		...
		if(pMsg->m_Team != TEAM_SPECTATORS && m_LockTeams)
		{
			...
			pPlayer->m_NoBroadcastTimer = 3 * Server()->TickSpeed();
			return;
		}

		if(pPlayer->m_IsDead != TEAM_SPECTATORS)
		{
			pPlayer->m_LastSetTeam = Server()->Tick();
			SendBroadcast("You have been killed by a player, wait the next round to respawn", ClientID);
			pPlayer->m_NoBroadcastTimer = 3 * Server()->TickSpeed();
			return;
		}
		...
		if(m_pController->CanJoinTeam(pMsg->m_Team, ClientID))
		{
			...
			{
				pPlayer->m_NoBroadcastTimer = 2*Server()->TickSpeed();;
	...
	else if (MsgID == NETMSGTYPE_CL_EMOTICON && !m_World.m_Paused)
	{
		...
		if(pChr && g_Config.m_SvEmotionalTees)
		{
			...
			if(pMsg->m_Emoticon == EMOTICON_OOP && pChr && !pChr->GetFreezeTicks())
			{
					
				int stt,st = pChr->GetSubActiveWeapon();
				stt = st;
				...
				pChr->SetSubActiveWeapon(st);
				if(stt != st) CreateSound(pChr->GetPosition(), SOUND_WEAPON_SPAWN, (1<<pPlayer->GetCID()));
			}
	...
	else if (MsgID == NETMSGTYPE_CL_KILL && !m_World.m_Paused)
	{
		...
		//if (pPlayer->GetCharacter() && pPlayer->GetCharacter()->GetFreezeTicks() > 0 || pPlayer->GetCharacter() && pPlayer->GetCharacter()->Get_Vel_Lag())
			//SendChatTarget(pPlayer->GetCID(), "You cannot commit suicide while being frozen or being lagy!");
		//else
			//pPlayer->KillCharacter(WEAPON_SELF);
		SendChatTarget(pPlayer->GetCID(), "Self Kill is Closed!");
	}
}
...
void CGameContext::ConTuneParam(IConsole::IResult *pResult, void *pUserData)
{
	...
	if(pSelf->Tuning()->Set(pParamName, NewValue))
	{
		...
		pSelf->Console()->Print(IConsole::OUTPUT_LEVEL_STANDARD, "tuning", "No such tuning parameter");*/ //Shield delete
...
void CGameContext::ConTuneReset(IConsole::IResult *pResult, void *pUserData)
{
	...
	*pSelf->Tuning() = TuningParams;
	pSelf->SendTuningParams(-1);
	pSelf->Console()->Print(IConsole::OUTPUT_LEVEL_STANDARD, "tuning", "Tuning reset");*/// must deleted
	CGameContext *pSelf = (CGameContext *)pUserData;
	/*CTuningParams TuningParams;
	*pSelf->Tuning() = TuningParams;
	pSelf->SendTuningParams(-1);
...
void CGameContext::ConTuneDump(IConsole::IResult *pResult, void *pUserData)
{
	...
	for(int i = 0; i < pSelf->Tuning()->Num(); i++)
	{
		...
		pSelf->Console()->Print(IConsole::OUTPUT_LEVEL_STANDARD, "tuning", aBuf); // Shield delete
	}*/
...
void CGameContext::ConAddSpecialVote(IConsole::IResult *pResult, void *pUserData)
{
	CGameContext *pSelf = (CGameContext *)pUserData;
	const char *pDescription = pResult->GetString(0);
	char aBuf[512];
	str_format(aBuf, sizeof(aBuf), "add_vote \"%s\" specialvote", pDescription);
...
void CGameContext::ConAddVote(IConsole::IResult *pResult, void *pUserData)
{
	...
	if(!str_comp_nocase(pCommand, "specialvote"))
    {
        // do nothing
    }
    else
    ...
    else if(str_length(pDescription) >= VOTE_DESC_LENGTH || *pDescription == 0)
    {
        char aBuf[256];
	...
	while(pOption)
	{
	    if(!str_comp_nocase(pCommand, "specialvote") && !pSelf->IsBasicSpecialVote(pDescription))
        {
            // do nothing
        }
        else if(str_comp_nocase(pDescription, pOption->m_aDescription) == 0)
		{
			char aBuf[256];
...
void CGameContext::ConRemoveVote(IConsole::IResult *pResult, void *pUserData)
{
	...
	if(pSelf->IsBasicSpecialVote(pDescription))
    {
        pSelf->Console()->Print(IConsole::OUTPUT_LEVEL_STANDARD, "server", "This vote can't be removed");
		return;
    }
...
void CGameContext::OnConsoleInit()
{
	...
	Console()->Register("add_svote", "s", CFGFLAG_SERVER, ConAddSpecialVote, this, "Add a special voting option");
...
void CGameContext::OnInit(/*class IKernel *pKernel*/)
{
	...
	else if(str_comp(g_Config.m_SvGametype, "CStrike") == 0)
...
void CGameContext::HandleOptions(int ClientID, const char* pDesc, const char* pReason)
{
	char aBuf[64];

    if(!str_comp(pDesc, "Informations about the mod"))
        ExecuteCommand(ClientID, "/info");
    else if(!str_comp(pDesc, "Help"))
        ExecuteCommand(ClientID, "/help");
    else{ str_format(aBuf, sizeof(aBuf), "Hammer ▶  Get Mines Hitter - %d points", g_Config.m_SvBuyMineHit);
	if(!str_comp(pDesc, aBuf))
		ExecuteCommand(ClientID, "/minehit");
	else{ str_format(aBuf, sizeof(aBuf), "Hammer ▶  Get Mines Freezer - %d points", g_Config.m_SvBuyMineFreeze);
	if(!str_comp(pDesc, aBuf))
		ExecuteCommand(ClientID, "/minefreeze");
	else{ str_format(aBuf, sizeof(aBuf), "Hammer ▶  Get Mines Killer - %d points", g_Config.m_SvBuyMineKill);
	if(!str_comp(pDesc, aBuf))
		ExecuteCommand(ClientID, "/minekill");
	else{ str_format(aBuf, sizeof(aBuf), "Hammer ▶  Get Flash - %d points", g_Config.m_SvBuyFlash);
	if(!str_comp(pDesc, aBuf))
		ExecuteCommand(ClientID, "/flash");
	else{ str_format(aBuf, sizeof(aBuf), "Hammer ▶  Get Smoke - %d points", g_Config.m_SvBuySmoke);
	if(!str_comp(pDesc, aBuf))
        ExecuteCommand(ClientID, "/smoke");
	else{ str_format(aBuf, sizeof(aBuf), "Hammer ▶  Get Grenade - %d points", g_Config.m_SvBuyGrenade);
	if(!str_comp(pDesc, aBuf))
        ExecuteCommand(ClientID, "/grenade");
	else{ str_format(aBuf, sizeof(aBuf), "Hammer ▶  Get Shield - %d points", g_Config.m_SvBuyShield);
	if(!str_comp(pDesc, aBuf))
        ExecuteCommand(ClientID, "/shield");
	else{ str_format(aBuf, sizeof(aBuf), "Gun ⊗  Get Glock  - %d points", g_Config.m_SvBuyGlock);
	if(!str_comp(pDesc, aBuf))
        ExecuteCommand(ClientID, "/glock");
	else{ str_format(aBuf, sizeof(aBuf), "Gun ⊗  Get Desert - %d points", g_Config.m_SvBuyDesert);
	if(!str_comp(pDesc, aBuf))
        ExecuteCommand(ClientID, "/desert");
	else{ str_format(aBuf, sizeof(aBuf), "Grenade ⊛  Get RPG - %d points", g_Config.m_SvBuyRPG);
	if(!str_comp(pDesc, aBuf))
        ExecuteCommand(ClientID, "/rpg");
	else{ str_format(aBuf, sizeof(aBuf), "Rifle ⊙  Get AWP - %d points", g_Config.m_SvBuyAWP);
	if(!str_comp(pDesc, aBuf))
        ExecuteCommand(ClientID, "/awp");
	else{ str_format(aBuf, sizeof(aBuf), "Rifle ⊙  Get AK47 - %d points", g_Config.m_SvBuyAK47);
	if(!str_comp(pDesc, aBuf))
        ExecuteCommand(ClientID, "/ak47");
	else{ str_format(aBuf, sizeof(aBuf), "Hammer ▶  Get Mines Hitter - %d points⠀", g_Config.m_SvBuyMineHit);
	if(!str_comp(pDesc, aBuf))
        ExecuteCommand(ClientID, "/minehit_c");
	else{ str_format(aBuf, sizeof(aBuf), "Hammer ▶  Get Mines Freezer - %d points⠀", g_Config.m_SvBuyMineFreeze);
	if(!str_comp(pDesc, aBuf))
        ExecuteCommand(ClientID, "/minefreeze_c");
	else{ str_format(aBuf, sizeof(aBuf), "Hammer ▶  Get Mines Killer - %d points⠀", g_Config.m_SvBuyMineKill);
	if(!str_comp(pDesc, aBuf))
        ExecuteCommand(ClientID, "/minekill_c");
	else{ str_format(aBuf, sizeof(aBuf), "Hammer ▶  Get Flash - %d points⠀", g_Config.m_SvBuyFlash);
	if(!str_comp(pDesc, aBuf))
        ExecuteCommand(ClientID, "/flash_c");
	else{ str_format(aBuf, sizeof(aBuf), "Hammer ▶  Get Smoke - %d points⠀", g_Config.m_SvBuySmoke);
	if(!str_comp(pDesc, aBuf))
        ExecuteCommand(ClientID, "/smoke_c");
	else{ str_format(aBuf, sizeof(aBuf), "Hammer ▶  Get Granade - %d points⠀", g_Config.m_SvBuyGrenade);
	if(!str_comp(pDesc, aBuf))
        ExecuteCommand(ClientID, "/grenade_c");
	else{ str_format(aBuf, sizeof(aBuf), "Hammer ▶  Get Shield - %d points⠀", g_Config.m_SvBuyShield);
	if(!str_comp(pDesc, aBuf))
        ExecuteCommand(ClientID, "/shield_c");
	else{ str_format(aBuf, sizeof(aBuf), "Gun ⊗  Get USP - %d points⠀", g_Config.m_SvBuyUSP);
	if(!str_comp(pDesc, aBuf))
        ExecuteCommand(ClientID, "/usp_c");
	else{ str_format(aBuf, sizeof(aBuf), "Gun ⊗  Get Desert - %d points⠀", g_Config.m_SvBuyDesert);
	if(!str_comp(pDesc, aBuf))
        ExecuteCommand(ClientID, "/desert_c");
	else{ str_format(aBuf, sizeof(aBuf), "Shotgun ⊛  Get FAMAS - %d points⠀", g_Config.m_SvBuyFAMAS);
	if(!str_comp(pDesc, aBuf))
        ExecuteCommand(ClientID, "/famas_c");
	else{ str_format(aBuf, sizeof(aBuf), "Rifle ⊙  Get AWP - %d points⠀", g_Config.m_SvBuyAWP);
	if(!str_comp(pDesc, aBuf))
        ExecuteCommand(ClientID, "/awp_c");
	else{ str_format(aBuf, sizeof(aBuf), "Rifle ⊙  Get M4a - %d points⠀", g_Config.m_SvBuyMP4a);
	if(!str_comp(pDesc, aBuf))
        ExecuteCommand(ClientID, "/m4a_c");
	}}}}}}}}}}}}}}}}}}}}}}}}
	
}

bool CGameContext::IsBasicSpecialVote(const char* pDesc)
{

    if(!str_comp(pDesc, "Informations about the mod"))
        return true;

    if(!str_comp(pDesc, "Help"))
        return true;

	char aBuf[64];

	str_format(aBuf, sizeof(aBuf), "Hammer ▶  Get Mines Hitter - %d points", g_Config.m_SvBuyMineHit);
    if(!str_comp(pDesc, aBuf))
        return true;

	str_format(aBuf, sizeof(aBuf),  "Hammer ▶  Get Mines Freezer - %d points", g_Config.m_SvBuyMineFreeze);
	if(!str_comp(pDesc, aBuf))
        return true;

	str_format(aBuf, sizeof(aBuf),  "Hammer ▶  Get Mines Killer - %d points", g_Config.m_SvBuyMineKill);
	if(!str_comp(pDesc, aBuf))
        return true;

    str_format(aBuf, sizeof(aBuf),  "Hammer ▶  Get Flash - %d points", g_Config.m_SvBuyFlash);
	if(!str_comp(pDesc, aBuf))
        return true;

    str_format(aBuf, sizeof(aBuf),  "Hammer ▶  Get Smoke - %d points", g_Config.m_SvBuySmoke);
	if(!str_comp(pDesc, aBuf))
        return true;

    str_format(aBuf, sizeof(aBuf),  "Hammer ▶  Get Grenade - %d points", g_Config.m_SvBuyGrenade);
	if(!str_comp(pDesc, aBuf))
        return true;

	str_format(aBuf, sizeof(aBuf),  "Hammer ▶  Get Shield - %d points", g_Config.m_SvBuyShield);
	if(!str_comp(pDesc, aBuf))
        return true;

    str_format(aBuf, sizeof(aBuf),  "Gun ⊗  Get Glock  - %d points", g_Config.m_SvBuyGlock);
	if(!str_comp(pDesc, aBuf))
        return true;

    str_format(aBuf, sizeof(aBuf),  "Gun ⊗  Get Desert - %d points", g_Config.m_SvBuyDesert);
	if(!str_comp(pDesc, aBuf))
        return true;

    str_format(aBuf, sizeof(aBuf),  "Grenade ⊛  Get RPG - %d points", g_Config.m_SvBuyRPG);
	if(!str_comp(pDesc, aBuf))
        return true;

    str_format(aBuf, sizeof(aBuf),  "Rifle ⊙  Get AWP - %d points", g_Config.m_SvBuyAWP);
	if(!str_comp(pDesc, aBuf))
        return true;

    str_format(aBuf, sizeof(aBuf),  "Rifle ⊙  Get AK47 - %d points", g_Config.m_SvBuyAK47);
	if(!str_comp(pDesc, aBuf))
        return true;
	...
	str_format(aBuf, sizeof(aBuf),  "Hammer ▶  Get Granade - %d points⠀", g_Config.m_SvBuyGrenade);
	if(!str_comp(pDesc, aBuf))
        return true;
	...
	str_format(aBuf, sizeof(aBuf),  "Gun ⊗  Get USP - %d points⠀", g_Config.m_SvBuyUSP);
	if(!str_comp(pDesc, aBuf))
        return true;
	...
	str_format(aBuf, sizeof(aBuf),  "Shotgun ⊛  Get FAMAS - %d points⠀", g_Config.m_SvBuyFAMAS);
	if(!str_comp(pDesc, aBuf))
        return true;
	...
	str_format(aBuf, sizeof(aBuf),  "Rifle ⊙  Get M4a - %d points⠀", g_Config.m_SvBuyMP4a);
	if(!str_comp(pDesc, aBuf))
        return true;
