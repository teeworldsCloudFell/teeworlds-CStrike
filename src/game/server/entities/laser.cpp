...
bool CLaser::HitCharacter(vec2 From, vec2 To)
{
	...
	if (OwnerChar)
	{
		...
		if(OwnerChar->GetPlayer()->GetTeam() != Hit->GetPlayer()->GetTeam())
		{
			GameServer()->CreateDeath(Hit->m_Pos, Hit->GetPlayer()->GetCID());
			Hit->SetLagTicks(Server()->TickSpeed()*g_Config.m_SvFlashLage);

			if(Server()->Tick()%5==0)
				Hit->IncreaseHealth(-1*g_Config.m_SvHealthFlash, m_Owner);
		}
	}
